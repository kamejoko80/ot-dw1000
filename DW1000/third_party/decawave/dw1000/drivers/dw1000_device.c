...
)
{
    ...
    {
      header_write[cnt++] = 0x80 | recordNumber ;           // bit-7 is WRITE operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
    }
    else
    {
#ifdef DWT_API_ERROR_CHECK
...
#endif
        header_write[cnt++] = 0xC0 | recordNumber ;       // bit-7 is WRITE operation, bit-6 one=sub-address follows, bits 5-0 is reg file id

        if (index <= 127)                                   // for non-zero index < 127, just a single sub-index byte is required
        {
            header_write[cnt++] = (uint8)index ;                  // bit-7 zero means no extension, bits 6-0 is index.
        }
        else
        {
            header_write[cnt++] = 0x80 | (uint8)(index) ;           // bit-7 one means extended index, bits 6-0 is low seven bits of index.
            header_write[cnt++] =  (uint8) (index >> 7) ;           // 8-bit value = high eight bits of index.
        }
    }
    ...
    // write it to the SPI
        memcpy(header_write+cnt,buffer,length);
        nrf_drv_spi_transfer(&spi,header_write,length+cnt,dummy,0);
        decamutexoff(stat);
...
)
{
    ...
    if (index == 0)                                         // for index of 0, no sub-index is required
    {
        header_read[cnt++] = (uint8) recordNumber ;              // bit-7 zero is READ operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
    }
    else
    {
#ifdef DWT_API_ERROR_CHECK
...
#endif
        header_read[cnt++] = (uint8)(0x40 | recordNumber) ;      // bit-7 zero is READ operation, bit-6 one=sub-address follows, bits 5-0 is reg file id

        if (index <= 127)                                   // for non-zero index < 127, just a single sub-index byte is required
        {
            header_read[cnt++] = (uint8) index ;                 // bit-7 zero means no extension, bits 6-0 is index.
        }
        else
        {
            header_read[cnt++] = 0x80 | (uint8)(index) ;           // bit-7 one means extended index, bits 6-0 is low seven bits of index.
            header_read[cnt++] =  (uint8) (index >> 7) ;           // 8-bit value = high eight bits of index.
        }
    }

        stat = decamutexon() ;

        memset(header_read+cnt,0,length);

        nrf_drv_spi_transfer(&spi,header_read,length+cnt,read_data,length+cnt);
        memcpy(buffer,read_data+cnt,length);
        decamutexoff(stat);
        return 0;
#else
    uint8 header[3] ;                                       // buffer to compose header in
